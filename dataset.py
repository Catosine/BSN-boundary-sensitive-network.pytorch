...
class VideoDataSet(data.Dataset):
    ...
    def _get_base_data(self,index):
        ...
        if self.customized_data:
            ...
            if x > self.interpolation_size:
                raise RuntimeError("x is greater than {}. please use larger interpolation size".format(self.interpolation_size))
            interp = interp2d(np.arange(y),np.arange(x),video_data)
            video_data = interp(np.arange(y),np.arange(self.interpolation_size))
            new_x, new_y = video_data.shape
            if new_x != self.interpolation_size or new_y != y:
                raise RuntimeError("Unexpected shape: now we have x as {} and y as {}".format(new_x, new_y))
