...
def train_PEM(data_loader, model, optimizer, epoch, writer, opt):
    ...
    for n_iter, (input_data, label_iou) in enumerate(data_loader):
        #print(model.conv1(input_data).shape)
        #exit(0)
...
def customized_collate_fn(batch):
    ...
    for item in batch:
        idx.append(item[0])
        data.append(item[1])
        anchor_xmin.append(item[2])
        anchor_xmax.append(item[3])
    
    return idx, torch.Tensor(data), anchor_xmin, anchor_xmax
...
def BSN_inference_TEM(opt):
    ...
    #                                           batch_size=model.module.batch_size, shuffle=False,
    #                                           num_workers=8, pin_memory=True, drop_last=False, collate_fn=customized_collate_fn)
    # else:
    #     test_loader = torch.utils.data.DataLoader(VideoDataSet(opt, subset="full"),
    #                                           batch_size=model.module.batch_size, shuffle=False,
